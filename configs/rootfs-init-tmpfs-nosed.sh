...
# Set tmpfs vars.
#
# karudeb replaces Debian's sed-based /lib/init/tmpfs.sh parser with this
# shell-only variant. The target UART bring-up path has shown sed POSIX
# character-class failures during early SysV boot, and these helpers only need
# simple numeric fields from /proc/meminfo and df(1).
...
tmp_free_space ()
{
    set -- $(LC_ALL=C df -kP /tmp 2>/dev/null | tail -n 1)
    [ "$#" -ge 4 ] || return 0
    case "$4" in
        ''|*[!0-9]*) return 0 ;;
        *) echo "$4" ;;
